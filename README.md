...
The manager chooses $\mathscr{A}$ to maximize utility:

$$
U_M(\mathscr{A} \mid \mathcal{I}_M) = \phi_1 P(\Omega) + \phi_2(\mathscr{A} - \hat{g}_M) - \frac{\psi_P}{2}(\mathscr{A} - \hat{g}_M)^2
\tag{3.8}
$$
...
**Disclosure Regime** ($\Omega = \{y_G, \mathscr{A}\}$):

The manager reports $\mathscr{A} = \hat{g}_M + B^*$. Rational investors filter out the equilibrium bias $B^*$:

$$
\hat{g}_M = E[\tilde{g} \mid y_G, \mathscr{A}] = \mathscr{A} - B^*
\tag{3.11}
$$
...
$$
P^{D} = (K + I_{0}) + y_{G} + (\mathscr{A} - B^{*}) - \lambda \Sigma_{D}
\tag{3.12}
$$

where $\Sigma_{D} \equiv \text{Var}(\tilde{e} \mid y_G, \mathscr{A})$. Disclosure strictly reduces posterior variance ($\Sigma_D < \Sigma_{ND}$). Define the uncertainty reduction ratio:
...
The manager chooses $\mathscr{A}$ to maximize:

$$
U_{M}^{D}(\mathscr{A}) = \phi_{1} \left[ (K + I_{0}) + y_{G} + (\mathscr{A} - B^{*}) - \lambda \Sigma_{D} \right] + \phi_{2} (\mathscr{A} - \hat{g}_{M}) - \frac{\psi_{P}}{2} (\mathscr{A} - \hat{g}_{M})^{2}
\tag{3.20}
$$
...
$$
\frac{\partial U_M^D}{\partial \mathscr{A}} = \phi_1 + \phi_2 - \psi_P(\mathscr{A} - \hat{g}_M) = 0
\tag{3.21}
$$

**Lemma 3.2 (Equilibrium Reporting Bias)**

Conditional on disclosure, the manager systematically overstates the Non-GAAP adjustment. The optimal bias $B^* \equiv \mathscr{A}^* - \hat{g}_M$ is:
...
2. **Equilibrium bias:** Conditional on disclosure, $\mathscr{A}^* = \hat{g}_M + B^*$ where:
...
The firm raises required debt financing $D_0$ at $t=0$ from a competitive credit market. The face value $L(\Omega)$ is determined endogenously based on public information $\Omega = \{y_G, \mathscr{A}\}$.
...
$$
\frac{\partial \Sigma_D}{\partial \mathscr{A}} \geq 0 \quad \text{and} \quad \frac{\partial^2 \Sigma_D}{\partial \mathscr{A}^2} \geq 0
\tag{4.4}
$$

**Lemma 4.2 (The Convex Cost of Debt)**

The equilibrium cost of debt $r_L(\mathscr{A})$ is increasing and convex in the magnitude of the Non-GAAP adjustment:

$$
\frac{\partial r_L}{\partial \mathscr{A}} > 0 \quad \text{and} \quad \frac{\partial^2 r_L}{\partial \mathscr{A}^2} > 0
\tag{4.5}
$$
...
The manager chooses $\mathscr{A}$ to maximize utility, now incorporating the cost of debt service:

$$
\max_{\mathscr{A}} \quad \phi_1 \left[ E[\widetilde{T}_t \mid \Omega] - (1 + r_L(\mathscr{A})) D_0 - \lambda \Sigma_D \right] + \phi_2(\mathscr{A} - \hat{g}_M) - \frac{\psi_P}{2}(\mathscr{A} - \hat{g}_M)^2
\tag{4.6}
$$
...
$$
\phi_1 (1 - r_L'(\mathscr{A}^*) D_0) + \phi_2 - \psi_P (\mathscr{A}^* - \hat{g}_M) = 0
\tag{4.7}
$$

The term $r_L'(\mathscr{A}^*) D_0$ represents the **marginal real cost** of reporting—as the manager inflates the signal, the yield on debt rises, transferring value from equity to debt.
...
where $\Delta_{\text{Debt}} = (r_L(\mathscr{A}^*) - r_L(0)) D_0 > 0$ represents the real cost of disclosure.

2. **Equilibrium Bias:** Conditional on disclosure, the optimal bias is strictly damped by leverage:

$$
B^* = \frac{\phi_1(1 - r_L'(\mathscr{A}^*) D_0) + \phi_2}{\psi_P}
\tag{4.9}
$$
...
where $\Delta r_L = r_L(\mathscr{A}^*) - r_L(0)$. The **Real Debt Cost** represents a value transfer from shareholders to creditors (Remark 4.2).
...
$$
D_0 < D^* \equiv \frac{\lambda(\Sigma_{ND} - \Sigma_D)}{r_L(\mathscr{A}^*) - r_L(0)}
\tag{4.16}
$$
...
$$
\Delta r_L = r_L(\mathscr{A}^*) - r_L(0) > 0
\tag{6.2}
$$
